...
namespace BasicWebApp.Tests
{
    public class UserServicesTests
    {
        ...
        [Theory]
        [MemberData(nameof(Sorting))]
        public static void OrdersUsersBySortByParameters(SortingData sortingData)
        {
            var usersToTest = new[]
            {
                new User
            ...
            var result = userService.GetAll(
                usersToTest.AsQueryable(), new UserQueryParameters() {SortBy = sortingData.SortBy, Order = sortingData.Order});
            var resultIds = GetIdsFromUsers(result);
            Assert.Equal(sortingData.SortedData, resultIds);
        }
        
        [Fact]
        public static void GetAllWithNoOrderButSortByInputOrdersUsingAsc()
        {
            var usersToTest = new[]
            {
                new User
            ...
            var result = userService.GetAll(
                usersToTest.AsQueryable(), new UserQueryParameters() {SortBy = "FirstName"});
            var resultIds = GetIdsFromUsers(result);
            Assert.Equal(expectedResult, resultIds);
        }
        ...
        {
            public Guid[] SortedData;
            public string SortBy;
            public string Order;
        }

        public static IEnumerable<object[]> Sorting =>
            new TheoryData<SortingData>
            {
                new SortingData()
                {
                    SortedData = new[]
